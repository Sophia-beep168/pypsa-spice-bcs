...
"""
Create Global Power input page.

Page displays and allows editing of global input tables for the power sector.
"""
...
def render_input_power_global_section(
    ...
    """Render global power input section with editable tables and charts.
...
def render_input_power_timeseries_section(
    ...
    """Render timeseries power input section with charts.
...
def render_input_power_demand_profile_section(
    ...
    """Render global power demand input section with editable charts.
...
if __name__ == "__main__":
    ...
    # Render country selection pills for the power global input section
    sector_selected_countries = render_countries_pills(
        all_countries=all_countries,
        key="power_global_pills",
    )

    # Render type and PyPSA class filters for the power global input section
    ...
    # Render country selection pills for the power demand profiles section
    demand_selected_countries = render_countries_pills(
        all_countries=all_countries,
        key="demand_power_global_pills",
    )

    # Render type and PyPSA class filters for the power demand profiles section
    demand_profile_types, demand_selected_classes = render_type_and_class_filters(
        tech_df,
        key="power_demand_global",
    )
